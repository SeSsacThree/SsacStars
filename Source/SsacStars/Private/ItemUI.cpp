...
void UItemUI::NativeConstruct()
{
	...
	CurrentPlayer = PartyGameState->CurrentPlayer;
...
{
	PartyGameState->ServerClickedItem1Button();
	/*
...
void UItemUI::Select2Button()
{
	...
	PartyGameState->ServerClickedItem2Button();
	/*
...
void UItemUI::SwitchItem(EItem SelectedItem)//아이템사용효과를 넣으면 된다
{
	...
	case EItem::Add3Dice:
	{
		CurrentPlayer->ToApplyDo = EItem::Add3Dice;
		break;
	}
	case EItem::WarpToStar:
	{
		CurrentPlayer->ToApplyDo = EItem::WarpToStar;
		break;
	}
	case EItem::SwitchCharacter:
	{
		CurrentPlayer->ToApplyDo = EItem::SwitchCharacter;
		break;
	}
	default:
	{
		CurrentPlayer->ToApplyDo = EItem::Nothing;
		break;
	}
...
void UItemUI::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(UItemUI, PlayerInventory);
}
